...
if __name__ == '__main__':
    ...
    equ_l = cv2.merge([cv2.equalizeHist(l), a, b])
    equ_res = np.hstack((img_color, cv2.cvtColor(equ_l, cv2.COLOR_LAB2BGR)))
    cv2.imshow('equ', equ_res)

    clahe_l = cv2.merge([cv2.createCLAHE(4, (8, 8)).apply(l), a, b])
    clahe_res = np.hstack((img_color, cv2.cvtColor(clahe_l, cv2.COLOR_LAB2BGR)))
